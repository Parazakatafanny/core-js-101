...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const [fir, sec] = [isStartIncluded ? '[' : '(', isEndIncluded ? ']' : ')'];
  return `${fir}${min}, ${max}${sec}`;
}
...
function isCreditCardNumber(ccn) {
  const numbers = ccn.toString();
  const parity = (numbers.length - 1) % 2;
  let sum = 0;
  for (let i = 0; i < numbers.length - 1; i += 1) {
    if (parity === 0) {
      if (i % 2 === 0) {
        ...
        if (doubleNum > 9) {
          sum += +doubleNum.toString()[0] + +doubleNum.toString()[1];
        } else {
          sum += doubleNum;
        }
      }
  ...
  return ((10 - (sum % 10)) % 10) === +numbers[numbers.length - 1];
}

/**
 * Returns the digital root of integer:
 *   step1 : find sum of all digitss
...
function getCommonDirectoryPath(pathes) {
  const parts = pathes.map((filename) => filename.split('/'));
  const path = parts[0].filter((value) => parts.slice(1).every((array) => array.includes(value)));
  if (path.length >= 1) {
    return `${path.join('/')}/`;
  }
  return path;
